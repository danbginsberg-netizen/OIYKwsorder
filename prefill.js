/* Pre-fill the order from a link, e.g. from the Capsule Builder:
   ...
   Each style's pieces go into its Dz / 6-pk / 3-pk boxes, largest pack first, using only the boxes that style offers;
   a quantity below the style's minimum is rounded up to it. &reorder=1 ticks "This is a reorder" (dozens only).
...
(function () {
  ...
  cart.split(",").forEach(function (t) {
    ...
    var prod = PRODUCTS[i], row = null, smallest = null;
    packs.forEach(function (pk) {
      var inp = prod[pk[0]] == null ? null : document.querySelector('input.qty[data-i="' + i + '"][data-k="' + pk[0] + '"]');
      if (!inp) return;                    // box not offered for this style (or below its minimum)
      smallest = inp;
      if (left < pk[1]) return;
      var n = Math.floor(left / pk[1]);
      inp.value = n; left -= n * pk[1]; row = inp.closest("tr");
    });
    if (left > 0 && smallest) {            // below the style's minimum pack: round up to it
      smallest.value = (parseInt(smallest.value) || 0) + 1; row = smallest.closest("tr"); rounded.push(sku);
    }
  ...
  note.textContent = "Pre-filled from your curated capsule: " + n + " style" + (n === 1 ? "" : "s") + " at the top of the list." +
    ...
    (rounded.length ? " Rounded up to the style's minimum: " + rounded.join(", ") + "." : "") +
